...
ActionInterval* CCBAnimationManager::getAction(CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const std::string& propName, Node *pNode)
{
    ...
    {
        return FadeTo::create(duration, 255 * pKeyframe1->getValue().asFloat());
    }
    ...
    else if (propName == "position")
    {
        ...
        Vec2 absPos = getAbsolutePosition(Vec2(x,y), type, xUnit, yUnit, containerSize, propName);
...
void CCBAnimationManager::setAnimatedProperty(const std::string& propName, Node *pNode, const Value& value, Ref* obj, float fTweenDuration)
{
    ...
    else 
    {
        ...
        if (propName == "position")
        {
            ...
            pNode->setPosition(getAbsolutePosition(Vec2(x,y), type, xUnit, yUnit, getContainerSize(pNode->getParent()), propName));
        }
        else if (propName == "scale")
        {
            // Get scale type
        ...
        else 
        {
            ...
            // TODO only handle rotation, opacity, spriteFrame, color
            ...
            {
                float opacity = value.asFloat();
                pNode->setOpacity(opacity*255);
            }
            else if (propName == "spriteFrame")
            {
...
void CCBAnimationManager::stopAnimationSeq()
{
    _rootNode->stopAllActions();
    for (auto nodeSeqIter = _nodeSequences.begin(); nodeSeqIter != _nodeSequences.end(); ++nodeSeqIter)
    {
...
void CCBAnimationManager::resumeAnimationSeq()
{
    if (_lastPlaySequenceId > 0)
    {
        this->runAnimationsForSequenceIdTweenDuration(_lastPlaySequenceId, 0);
    }
}
