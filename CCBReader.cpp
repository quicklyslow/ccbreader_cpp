...
Node* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, Ref *pOwner, const Size &parentSize)
{
    ...
    if (this->_nameNode != nullptr) {
        ret->setUserObject(_nameNode);
    }
...
Node * CCBReader::readNodeGraph(Node * pParent)
{
    ...
    // Assign controller
    if( _animationControllerName.length() > 0 && node == _animationManager->getRootNode())
    {
        _animationManager->setDocumentControllerName(_animationControllerName);
    }
	...
	unsigned int uuid = readInt(false);
...
CCBKeyframe* CCBReader::readKeyframe(PropertyType type)
{
    ...
    {
        unsigned char r = readFloat() * 255;
		unsigned char g = readFloat() * 255;
		unsigned char b = readFloat() * 255;
		unsigned char a = readFloat() * 255;
...
void CCBReader::setAnimationControllerName(const std::string &name)
{
    _animationControllerName = name;
}
    
void CCBReader::addNamedCallbackNode(const std::string& name, cocos2d::Node* node)
{
    if (_nameNode == nullptr) {
        _nameNode = new CCBNamedNode();
        _nameNode->autorelease();
    }
    _nameNode->addNodeWithName(name, node);
}
